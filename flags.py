...
flags.DEFINE_bool("continue_training", False, "Continue training where it stopped")
flags.DEFINE_string("model_dir", "model", "model_dir (containing ckpt files and word_to_id)")
flags.DEFINE_string("model", "default", "model_type")
flags.DEFINE_string( "config", None, "A type of model. Possible options are: 'small', 'medium', 'large'")
flags.DEFINE_string("data_path", None,
                    "Where the training/test data is stored.")
flags.DEFINE_string("loss", "softmax", 
                    "The loss function to use. Possible options are %s" % ", ".join(LOSS_FCTS))
flags.DEFINE_string("loss_fct", "softmax", 
                    "The loss function to use. Possible options are %s" % ", ".join(LOSS_FCTS))
...
flags.DEFINE_string("cell", "gru", "Choose between LSTM (default) and GRU")
MODEL_PARAMS_INT = [
      "max_grad_norm",
      "num_layers",
      "hidden_size",
